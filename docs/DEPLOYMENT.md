...
FROM sys.assemblies- Neo4j 5.x (for graph features)

WHERE name IN ('HartonomousSqlBridge', 'SqlClrFunctions');

- Azure subscription (for Event Hubs CDC integration)**Azure Resources (for production):**

SELECT is_broker_enabled 

FROM sys.databases - Docker (for containerized deployment)- Azure Key Vault (credential management)
...
JOIN sys.query_store_runtime_stats rs ON p.plan_id = rs.plan_idWHERE name IN ('HartonomousSqlBridge', 'SqlClrFunctions');
