...
if [ -z "$RUNNER_USER" ]; then
    echo "⚠️ Warning: Could not detect GitHub Actions runner user"
...
if command -v dotnet &> /dev/null; then
    ...
    if [ "$major_version" -ge 10 ]; then
        ...
        echo " ⚠️ Version $dotnet_version is too old (need 10.0+)"
...
if command -v pwsh &> /dev/null; then
    ...
    if [ -f /etc/os-release ]; then
        ...
        if [[ "$ID" == "ubuntu" ]]; then
            ...
            echo "      ⚠️ Manual installation required for $ID"
            echo "      See: https://aka.ms/powershell-release?tag=stable"
            checks+=("PowerShell|⚠️|Manual install needed")
            warn_count=$((warn_count + 1))
        fi
    else
        echo "      ⚠️ Cannot detect OS, manual installation required"
        checks+=("PowerShell|⚠️|Manual install needed")
        warn_count=$((warn_count + 1))
    fi
fi
...
if command -v docker &> /dev/null; then
    ...
    if sudo -u $RUNNER_USER docker ps &> /dev/null; then
        ...
        echo " ⚠️ $docker_version (fixing permissions...)"
        echo "      Adding $RUNNER_USER to docker group..."
        
        sudo usermod -aG docker $RUNNER_USER
        
        echo "      ✓ User $RUNNER_USER added to docker group"
        echo "      ℹ️ Runner service will be restarted to apply group changes"
...
if systemctl --user is-active --quiet actions.runner.* 2>/dev/null || systemctl is-active --quiet actions.runner.* 2>/dev/null; then
    ...
    echo " ⚠️ Not running or not installed"
    echo "      ℹ️ Runner requires manual setup with GitHub token"
    echo "      See: docs/ci-cd/RUNNER-SETUP.md"
    checks+=("GitHub Runner|⚠️|Manual setup needed")
    warn_count=$((warn_count + 1))
fi
...
if [ $warn_count -gt 0 ]; then
    echo "  ⚠️ Manual setup needed: $warn_count components"
fi
echo ""
...
if [ $fail_count -eq 0 ] && [ $warn_count -eq 0 ]; then
    ...
    echo ""
    echo "ℹ️ GitHub Actions runner service has been restarted to apply changes"
    exit 0
elif [ $fail_count -eq 0 ]; then
    echo "⚠️ SETUP COMPLETE with $warn_count warnings"
