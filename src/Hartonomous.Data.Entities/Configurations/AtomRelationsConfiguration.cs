...
public class AtomRelationsConfiguration : IEntityTypeConfiguration<AtomRelations>
{
    public void Configure(EntityTypeBuilder<AtomRelations> builder)
    {
        ...
        builder.HasOne(d => d.SourceAtom)
            .WithMany(p => p.AtomRelationsSourceAtom)
            .HasForeignKey(d => new { d.SourceAtomId })
            ;

        builder.HasOne(d => d.TargetAtom)
            .WithMany(p => p.AtomRelationsTargetAtom)
